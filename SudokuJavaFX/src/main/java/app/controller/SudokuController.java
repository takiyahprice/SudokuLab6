...
public class SudokuController implements Initializable {
	...
	private static final DataFormat myTrashCan = new DataFormat("com.cisc181.Trashcan");
	...
	private GridPane BuildNumbersGrid() {
			...
			paneSource.setOnDragDetected(new EventHandler<MouseEvent>() {
				public void handle(MouseEvent event) {
					...
					if (game.getSudoku().isPuzzleMaxMistakes()) {
						db = paneSource.startDragAndDrop(TransferMode.NONE);
					} else {
						db = paneSource.startDragAndDrop(TransferMode.ANY);
					}	
		...
		////
		StackPane paneTrash = new StackPane();
		//paneTrash.getChildren().add(new ImageView(GetImage(1)));
		ImageView ivTC = new ImageView(this.GetTrashcanImage());
		ivTC.setFitHeight(90);
		ivTC.setFitWidth(90);
		paneTrash.getChildren().add(ivTC);
		
		
		paneTrash.setOnDragDropped(new EventHandler<DragEvent>() {
		...
		paneTrash.setOnDragOver(new EventHandler<DragEvent>() {
			public void handle(DragEvent event) {
				if (event.getGestureSource() != paneTrash && event.getDragboard().hasContent(myTrashCan)) {
		...
		gridPaneNumbers.add(paneTrash, s.getiSize(), 0);
			...
			for (int iRow = 0; iRow < s.getiSize(); iRow++) {
					...
					public void handle(DragEvent event) {
						if (event.getGestureSource() != paneTarget && event.getDragboard().hasContent(myFormat)) {
							if (paneTarget.getCell().getiCellValue() == 0) {
								...
								if (game.getShowHints() && !s.isValidValue(paneTarget.getCell().getiRow(), paneTarget.getCell().getiCol(), ((Cell) event.getDragboard().getContent(myFormat)).getiCellValue())) {
				...
				paneTarget.setOnDragDetected(new EventHandler<MouseEvent>() {
					public void handle(MouseEvent event) {
						...
						if (/*game.getSudoku().isPuzzleMaxMistakes() ||*/ !paneTarget.getCell().isDraggable()) {
							db = paneTarget.startDragAndDrop(TransferMode.NONE);
						}
						else {
							db = paneTarget.startDragAndDrop(TransferMode.ANY);
						}	
				...
				paneTarget.setOnDragDropped(new EventHandler<DragEvent>() {
					public void handle(DragEvent event) {
						...
						if (db.hasContent(myFormat)) {
							...
							success = true;
							//allow player to remove guess after it has been added
							paneTarget.getCell().setbDraggable(true);
	...
	private Image GetTrashcanImage() {
		InputStream is = getClass().getClassLoader().getResourceAsStream("img/trashcan2.png");
		return new Image(is);
	}
